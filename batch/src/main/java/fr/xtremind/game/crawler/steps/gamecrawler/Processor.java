...
public class Processor implements ItemProcessor<Console, Console> {
	...
	public Processor(String requiredProperty/*, RestTemplate restTemplate*/) {
		if (logger.isInfoEnabled()) {
			logger.info("process " + requiredProperty);
		}
	...
	public Console process(Console console) {
		...
		while (this.nextCursor != null) {
			if (logger.isInfoEnabled()) {
				logger.info("********* index " + String.format(apiUrl, console.toString(), this.nextCursor));
			}
	...
	private List<Game> fetchGameDTOFromAPI(Console console) {
		//ResponseEntity<GameDTO> response = restTemplate.getForEntity(String.format(apiUrl, console.toString(), this.nextCursor), GameDTO.class);

		ResponseEntity<GameDTO> responseData = WebClient.create().get().uri(String.format(apiUrl, console.toString(), this.nextCursor)).exchange().flatMap(response -> response.toEntity(GameDTO.class)).block();
		
	
		GameDTO gameDTO = responseData.getBody();
